...
std::string joinStringsAccumulate(std::vector<std::string>&& strings) {
  return std::accumulate(std::make_move_iterator(strings.begin()),
                         std::make_move_iterator(strings.end()), std::string());
}
...
std::string addresses_to_string(
...
boost::asio::ip::address extract_valid_ip(
    const std::vector<boost::asio::ip::address>& addresses) {
  for (const auto& address : addresses) {
    if (!address.is_v4())
      continue;  // Skip non-IPv4 addresses
    std::string ip_string = address.to_string();
    if (ip_string.substr(0, 4) !=
        "127.") {  // Exclude addresses starting with "127."
      return address;
    }
  }
...
std::string extract_valid_ip_string(
    const std::vector<boost::asio::ip::address>& addresses) {
  for (const auto& address : addresses) {
    if (!address.is_v4())
      continue;  // Skip non-IPv4 addresses
...
std::vector<std::string> convertPacketToStrings(
    const std::array<char, 12>& header,
    const std::array<char, 64>& identify,
    bool useHex = true) {
  ...
  for (size_t i = 0; i < header.size(); ++i) {
    if (useHex) {
      headerStream << std::hex << std::setw(2) << std::setfill('0')
  ...
  for (size_t i = 0; i < identify.size(); ++i) {
    if (useHex) {
      identifyStream << std::hex << std::setw(2) << std::setfill('0')
                     << static_cast<int>(
                            static_cast<unsigned char>(identify[i]));
    } else {
      identifyStream << static_cast<int>(
          static_cast<unsigned char>(identify[i]));
    }
...
std::vector<boost::asio::ip::address> get_local_interfaces() {
  ...
  if (err == NO_ERROR) {
    for (AddrList addr = ifaddrs; addr != 0; addr = addr->Next) {
      ...
      if (addr->Ipv4Enabled) {
        for (Addr* uaddr = addr->FirstUnicastAddress; uaddr != 0;
             uaddr = uaddr->Next) {
          if (uaddr->Address.lpSockaddr->sa_family != AF_INET)
            continue;
          res.push_back(boost::asio::ip::make_address_v4(
              ntohl(reinterpret_cast<sockaddr_in*>(addr->ifa_addr)
                        ->sin_addr.s_addr)));
        }
      }
...
void checkForNewEntry(SharedVector& sharedVec,
                      size_t& lastSize,
                      Elements children) {
...
void monitorVector(SharedVector& sharedVec, Elements children) {
  ...
  while (true) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(100));  // Sleep for 100 ms before next check
  }
}
