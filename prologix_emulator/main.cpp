...
std::string udp_server(boost::asio::io_context& io_context,
  ...
  if (command == "start") {
    if (!batman.load()) {
      ...
      io_thread =
          std::make_unique<std::thread>([&io_context]() { io_context.run(); });
...
int main() {
  ...
  auto udpStart = [&] {
    ...
    sharedVec.vec.push_back(
        udp_server(udpcontext, udpthread, udpserver, "start", sharedVec));
  };

  auto udpStop = [&] {
    std::lock_guard<std::mutex> lock(sharedVec.vecMutex);
    sharedVec.vec.push_back(
        udp_server(udpcontext, udpthread, udpserver, "stop", sharedVec));
  };
  auto tcpStart = [&] {
    std::lock_guard<std::mutex> lock(sharedVec.vecMutex);
    sharedVec.vec.push_back(tcp_control(tcpcontext, tcpthread, tcpserver,
                                        "start", cmd, sharedVec, clientVec));
  };

  auto tcpStop = [&] {
    std::lock_guard<std::mutex> lock(sharedVec.vecMutex);
    sharedVec.vec.push_back(tcp_control(tcpcontext, tcpthread, tcpserver,
                                        "stop", cmd, sharedVec, clientVec));
  };
  auto Exit = [&] {
    std::lock_guard<std::mutex> lock(sharedVec.vecMutex);
    sharedVec.vec.push_back(
        udp_server(udpcontext, udpthread, udpserver, "exit", sharedVec));
    sharedVec.vec.push_back(tcp_control(tcpcontext, tcpthread, tcpserver,
                                        "exit", cmd, sharedVec, clientVec));
    running = false;
    screen.Exit();
  };
  ...
  auto cliArea =
      Renderer(cli_add,
               [&] {
                 return window(text("cli input"), vbox(cli_add->Render())) |
                        color(Color::Purple4);
               }) |
      size(HEIGHT, EQUAL, 1);
  ...
  auto udpbuttons =
      Container::Horizontal({
          Button("Start", udpStart) | center |
              color(Color::Green),                               // turn udp on
  ...
  auto output = Renderer([&] {
    ...
    for (size_t i = std::max(0, (int)sharedVec.vec.size() - 12);
         i < sharedVec.vec.size(); ++i) {
      std::lock_guard<std::mutex> lock(sharedVec.vecMutex);
      children.push_back(text(sharedVec.vec[i]));
    }
    return window(text("Output"),
                  vbox(children) | color(Color::Yellow4) | flex) |
           color(Color::Yellow4);
  });

  // -- Layout -----------------------------------------------------------------
  auto layout = Container::Vertical({checkframe,
                                     //        toggle,
                                     input_add, udpbuttons, tcpbuttons}) |
                size(HEIGHT, GREATER_THAN, 40);

  auto component = Renderer(layout, [&] {
    return vbox({checkframe->Render(), separator() | color(Color::Default),
                 udpbuttons->Render(), separator() | color(Color::Default),
                 tcpbuttons->Render()}) |
           size(WIDTH, GREATER_THAN, 30) |
           borderStyled(ROUNDED, Color::Default);
  });
  ...
  // create a new thread to run the output continously/force redraw. Required
  // for the output window to update on it's own instead of waiting for an
  // event.
