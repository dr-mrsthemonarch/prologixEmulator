...
void UDPServer::start_receive(SharedVector& sharedVec) {
  socket_.async_receive_from(
      ...
      boost::bind(
          ...
          boost::asio::placeholders::bytes_transferred, boost::ref(sharedVec)));
}
...
void UDPServer::handle_receive(const boost::system::error_code& error,
  ...
  std::string extracted_ip = extract_valid_ip_string(
      vector_ip);  // vector_ip is a list of all address at the interface,
                   // return only the valid one.
  ...
  if (!error || error == boost::asio::error::message_size) {
    ...
    // std::cout << "Received identify from " << hostname << "/" << port <<
    // std::endl;
    ...
    std::memcpy(identify_reply.data() + 8, ip_bytes.data(),
                4);  // ip addr: 10.10.10.10
    ...
    send_buffer.insert(send_buffer.end(), identify_reply.begin(),
                       identify_reply.end());
    // Convert packet to vector of strings
    std::vector<std::string> packetStrings =
        convertPacketToStrings(header, identify_reply, false);
