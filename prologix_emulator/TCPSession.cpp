...
//        });
//}
TCPSession::TCPSession(ip::tcp::socket socket, Command& cmd, SharedVector& sharedVec, SharedVector& clientVec)
    ...
    timer_.async_wait([this, self](boost::system::error_code ec) {
        if (!ec) {
            ...
            if (socket_.is_open()) {
                ...
                check_connection();  // Recursive call with captured 'self'
                std::lock_guard<std::mutex> lock(sharedVec_.vecMutex);
